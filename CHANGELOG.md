...
* [FEATURE][rust,store] Added PSWAP chain tracking: a new `pswap_lineages` SQLite table persists one row per local-creator PSWAP order, and `Client::sync_chain` follows each lineage across foreign fills via a `PswapChainObserver` (registered through the new `NoteObserver` trait on `StateSync`) plus a post-sync `discover_pswap_rounds` correlator. New public API: `Client::pswap_lineages`, `pswap_lineages_for`, `pswap_lineage`, `build_pswap_cancel_by_order`. New `Store` trait methods: `upsert_pswap_lineage`, `get_pswap_lineage`, `list_pswap_lineages`, `apply_pswap_round`. Reconstructed paybacks land in `input_notes` (in `Unverified` state with the captured inclusion proof) so the creator's normal consume flow finds them. WASM/IndexedDB backend stubs the four new store methods with `NotImplemented` for v1 ([#TBD](https://github.com/0xMiden/miden-client/pull/TBD)).
* [FEATURE][store] Added `pswap_lineages` SQLite migration (auto-detected by the hash-based migrator) with indexes on `state` and `current_tip_nullifier` for sync-time lookups ([#TBD](https://github.com/0xMiden/miden-client/pull/TBD)).
* [FEATURE][rust] Added `NoteTagSource::PswapAssetPair(Felt)` variant (on-disk discriminant `3`) for asset-pair tags registered by PSWAP lineage tracking. Tags are inserted at lineage creation and removed atomically when the lineage transitions to `FullyFilled` or `Reclaimed` ([#TBD](https://github.com/0xMiden/miden-client/pull/TBD)).
* [FEATURE][rust] Added `NoteObserver` trait and a `StateSync::with_note_observer(...)` builder for attaching side-effect-only per-note hooks during sync. Independent of the existing `OnNoteReceived` (Commit/Insert/Discard) decision; observer failures are logged via `tracing` and never block sync ([#TBD](https://github.com/0xMiden/miden-client/pull/TBD)).
