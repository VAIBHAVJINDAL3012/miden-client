...
fn create_consumed_input_note_with_consumer(
    ...
    // TEMP-PROTOCOL-ADAPTER: protocol restructured NoteMetadata; build via
    // PartialNoteMetadata + NoteAttachments. REVERT-WHEN: upstream adapter lands.
    let partial =
        PartialNoteMetadata::new(consumer, NoteType::Public).with_tag(NoteTag::from(index));
    let metadata = NoteMetadata::new(partial, &NoteAttachments::default());
